...
/**
 *	\file       htdocs/core/modules/commande/doc/pdf_fracastor.modules.php
 *	\ingroup    order
 *	\brief      File of Class to generate PDF orders with template Fracastor
 */
...
/**
 *	Class to generate PDF orders with template Fracastor
 */
class pdf_fracastor extends ModelePDFCommandes
{
	/**
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->db = $db;
		$this->name = "fracastor";
		$this->description = $langs->trans('PDFFracastorDescription');
		...
		if (getMultidirOutput($object)) {
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					// Stock
					if ($this->getColumnStatus('stock')) {
						$stock = $this->pdf_getlinestock($object, $i);
						$this->printStdColumnContent($pdf, $curY, 'stock', $stock);
						$nexY = max($pdf->GetY(), $nexY);
					}
